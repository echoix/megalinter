...
"""
    ...
    def on_page_content(self, html, page, config, **kwargs):
        ...
        log.warning("Starting on_page_content")
        # skip page with meta glightbox is false
        if "glightbox" in page.meta and page.meta.get("glightbox", True) is False:
            return html
        ...
        pattern = re.compile(
            r"<a\b[^>]*>(?:\s*<[^>]+>\s*)*<img\b[^>]*>(?:\s*<[^>]+>\s*)*</a>|<img(?P<attr>.*?)>", flags=re.DEBUG
        )
        # Use regex to find image tags that need to be wrapped with anchor tags and image tags already wrapped with anchor tags
 
        # pattern = re.compile(
        #     r"<a\b[^>]*>(?:\s*<[^>]+>\s*)*<img\b[^>]*>(?:\s*<[^>]+>\s*)*</a>|<img\b([^>]*?)>", flags=re.DEBUG
        # )
 
        # pattern = re.compile(
        #     r'<a\b[^>]*>(?:\s*<[^>]+>\s*)*?<img\b(?P<attr1>[^>]*)>(?:\s*<[^>]+>\s*)*?</a>|<img\b(?P<attr>[^>]*)>', flags=re.DEBUG
        # )
        # pattern = re.compile(
        #     r'<a\b[^>]*>(?:\s*<[^>]+>\s*)*?<img\b(?P<attr1>[^>]*)>(?:\s*<[^>]+>\s*)*?</a>|<img\b(?P<attr>[^>]*)>', # flags=re.DEBUG
        # )
        # log.warning("Starting on_page_content replace, page: %s, config: %s", page, config)
        log.warning("Starting on_page_content replace, page: %s, html len: %s, pattern flags: %s (re.DEBUG: %d)", page, len(html), pattern.flags, re.DEBUG)
        # log.warning("Matches: %s", pattern.findall(html))
